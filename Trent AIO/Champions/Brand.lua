...
if player.charName ~= "Brand" then
    print("not using Brand not loading")
    return
end
local menu = menu("Trent_Brandlelele", "Trent Brand")
				...
				if target.pos:dist(player.pos) < spellR.range then
					if not menu.combo.minion:get() then
						...
						if menu.combo.rmode:get() == 2 then
							if
								target.health <
									(WDamage(target) + QDamage(target) + RDamage(target) +
										EDamage(target))
							 then
								if (#count_enemies_in_range(target.pos, 750) >= menu.combo.hitr:get()) then
									player:castSpell("obj", 3, target)
...
local ELevelDamage = {70, 95, 120, 145, 170}
function EDamage(target)
	local damage = 0
	if player:spellSlot(2).level > 0 then
		damage =
			common.CalculateMagicDamage(target, (ELevelDamage[player:spellSlot(2).level] + (common.GetTotalAP() * .45)), player)
	end
	return damage
end

local WLevelDamage = {75, 120, 165, 210, 255}
function WDamage(target)
	local damage = 0
	if player:spellSlot(1).level > 0 then
		damage =
			common.CalculateMagicDamage(target, (WLevelDamage[player:spellSlot(1).level] + (common.GetTotalAP() * .60)), player)
	end
	return damage
end

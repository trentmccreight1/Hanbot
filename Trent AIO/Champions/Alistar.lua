...
if player.charName ~= "Alistar" then
    print("not using Alistar not loading")
    return
end
local menu = menu("Trent_Alistarlelele", "Trent Alistar")
	...
	if menu.combo.rcombo:get() then
		if (#count_enemies_in_range(player.pos, 600) >= menu.combo.hitr:get()) then
			if (player.health / player.maxHealth) * 100 <= menu.combo.hp:get() then
				if
					(player.buff[6] or player.buff[9] or player.buff[25] or player.buff[12] or player.buff[23] or player.buff[9] or
						player.buff[22])
				 then

...
menu.combo.rset:dropdown("rusage", "R Usage", 1,
                         {"At X Health", "Only if Killable", "Never"})
menu.combo.rset:slider("waster", "Don't waste R if Enemy Health < X", 100, 0,
                       500, 1)
...
menu.killsteal:slider("saver", "Don't waste R if Enemy Health < X", 100, 0, 500,
                      1)
    ...
    if enemy and interruptableSpells[name] then
        for v = 1, #interruptableSpells[name] do
            ...
            menu.misc.interrupt.interruptmenu:boolean(
                string.format(tostring(enemy.charName) ..
                                  tostring(spell.menuslot)), "Interrupt " ..
                    tostring(enemy.charName) .. " " .. tostring(spell.menuslot),
                true)
        end
    end
end
...
function count_enemies_in_something_range(pos, range)
    ...
    for i = 0, objManager.enemies_n - 1 do
        ...
        if pos:dist(enemy.pos) < range and common.IsValidTarget(enemy) and
            player.pos:dist(enemy.pos) < spellR.range and
            player.pos:dist(enemy.pos) > 370 then
...
local GetTarget = function() return ts.get_result(TargetSelection).obj end
...
local function IsFacing(target)
    return player.path.serverPos:distSqr(target.path.serverPos) >
               player.path.serverPos:distSqr(
                   target.path.serverPos + target.direction)
end
...
function EDamage(target)
    ...
    if player:spellSlot(2).level > 0 then
        if (target.buff["poisontrailtarget"] or target.buff["TwitchDeadlyVenom"] or
            target.buff["cassiopeiawpoison"] or target.buff["cassiopeiaqdebuff"] or
            target.buff["ToxicShotParticle"] or target.buff["bantamtraptarget"]) then
            damage = CalcMagicDmg(target,
                                  (((52 + ElvlDamage * (player.levelRef - 1)) +
                                      (common.GetTotalAP() * .1)) +
                                      ElvlDmgBonus[player:spellSlot(2).level] +
                                      (common.GetTotalAP() * .6)))
        else
            damage = CalcMagicDmg(target, ((52 + ElvlDamage *
                                      (player.levelRef - 1)) +
                                      (common.GetTotalAP() * .1)))
        end
    end
...
function CalcMagicDmg(target, amount, from)
    ...
    local targetMR =
        target.spellBlock * math.ceil(from.percentMagicPenetration) -
            from.flatMagicPenetration
    local dmgMul = 100 / (100 + targetMR)
    if dmgMul < 0 then dmgMul = 2 - (100 / (100 - targetMR)) end
...
local function AutoDash()
    ...
    if target then
        local pred_pos = preds.core.lerp(target.path,
                                         network.latency + spellQ.delay,
                                         target.path.dashSpeed)
    ...
    if player:spellSlot(3).state == 0 and menu.misc.GapA:get() then
        for i = 0, objManager.enemies_n - 1 do
            ...
            if dasher.type == TYPE_HERO and dasher.team == TEAM_ENEMY then
                if dasher and common.IsValidTarget(dasher) and
...
local GetTargetFR = function() return ts.get_result(TargetSelectionFR).obj end
                ...
                if (target.pos:dist(player.pos) <= spellR.range + 410) then
                    if (FlashSlot and player:spellSlot(FlashSlot).state) then
                        if (target.pos:dist(player.pos) > spellR.range) then
                            if (menu.combo.flashrface:get()) and
                                IsFacing(target) then
                                local pos =
                                    preds.linear.get_prediction(spellR, target)
                                if pos and pos.startPos:dist(pos.endPos) <
                                    spellR.range + 410 then
                                    player:castSpell("pos", 3, vec3(
                                                         pos.endPos.x,
                                                         mousePos.y,
                                                         pos.endPos.y))

                                    common.DelayAction(function()
                                        player:castSpell("pos", FlashSlot,
                                                         target.pos)
...
local function GetQDmg(unit)
    ...
    local damage = (({70, 110, 145, 180, 215})[player:spellSlot(0).level] +
                       (0.9 * common.GetTotalAP()))
    return common.CalculateMagicDamage(unit, damage)
end

local function GetRDmg(unit)
    if player:spellSlot(3).level < 1 or not unit or unit.isDead or
        not unit.isVisible or not unit.isTargetable then return 0 end
    local damage = (({100, 250, 350})[player:spellSlot(3).level] +
                       (0.5 * common.GetTotalAP()))
    return common.CalculateMagicDamage(unit, damage)
end
...
function KillSteal()
    ...
    for i, enemies in ipairs(enemy) do
        if enemies and enemies.isVisible and common.IsValidTarget(enemies) and
            not common.CheckBuffType(enemies, 18) then
            local hp = common.GetShieldedHealth("ap", enemies)
            if menu.killsteal.ksq:get() then
                if player:spellSlot(0).state == 0 and
                    vec3(enemies.x, enemies.y, enemies.z):dist(player) <
                    spellQ.range and GetQDmg(enemies) > hp then
                    local pos = preds.circular.get_prediction(spellQ, enemies)
                    if pos and pos.startPos:dist(pos.endPos) < spellQ.range then
                        player:castSpell("pos", 0, vec3(pos.endPos.x,
                                                        mousePos.y, pos.endPos.y))
                    end
                end
            end
            if menu.killsteal.kse:get() then
                if player:spellSlot(2).state == 0 and
                    vec3(enemies.x, enemies.y, enemies.z):dist(player) <
                    spellE.range and EDamage(enemies) > hp then
                    player:castSpell("obj", 2, enemies)
                end
            end
            if menu.killsteal.ksr:get() then
                if player:spellSlot(3).state == 0 and
                    vec3(enemies.x, enemies.y, enemies.z):dist(player) <
                    spellR.range and hp < GetRDmg(enemies) and
                    hp > menu.killsteal.saver:get() then
                    local pos = preds.linear.get_prediction(spellR, enemies)
                    if pos and pos.startPos:dist(pos.endPos) < spellR.range then
                        player:castSpell("pos", 3, vec3(pos.endPos.x,
                                                        mousePos.y, pos.endPos.y))
                    end
                end
            end
...
local function LastHit()
    if menu.lasthit.qlasthit:get() then
        for i = 0, objManager.minions.size[TEAM_ENEMY] - 1 do
            ...
            if minion and minion.isVisible and minion.moveSpeed > 0 and
                minion.isTargetable and not minion.isDead and
                minion.pos:dist(player.pos) < spellE.range then
                local minionPos = vec3(minion.x, minion.y, minion.z)
                -- delay = player.pos:dist(minion.pos) / 3500 + 0.2
                delay = 125 / 1000 + player.pos:dist(minion.pos) / 840
                if (EDamage(minion) >=
                    orb.farm.predict_hp(minion, delay / 2, true) - 150 and
                    player.mana > player.manaCost2) then
                    orb.core.set_pause_attack(1)
                end
                if (EDamage(minion) >=
                    orb.farm.predict_hp(minion, delay / 2, true)) then
    ...
    if (player.mana / player.maxMana) * 100 >= menu.laneclear.jungle.mana:get() then
        if menu.laneclear.jungle.useq:get() then
            for i = 0, objManager.minions.size[TEAM_NEUTRAL] - 1 do
                ...
                if minion and not minion.isDead and minion.moveSpeed > 0 and
                    minion.isTargetable and minion.isVisible and minion.type ==
                    TYPE_MINION then
                    if minion.pos:dist(player.pos) <= spellQ.range then
                        local pos = preds.circular
                                        .get_prediction(spellQ, minion)
                        if pos and pos.startPos:dist(pos.endPos) < spellQ.range then
                            player:castSpell("pos", 0, vec3(pos.endPos.x,
                                                            mousePos.y,
                                                            pos.endPos.y))
...
local function Harass()
    if menu.harass.laste:get() then
        for i = 0, objManager.minions.size[TEAM_ENEMY] - 1 do
            local minion = objManager.minions[TEAM_ENEMY][i]
            if minion and minion.isVisible and not minion.isDead and
                minion.pos:dist(player.pos) < spellE.range then
                local minionPos = vec3(minion.x, minion.y, minion.z)
                -- delay = player.pos:dist(minion.pos) / 3500 + 0.2
                delay = 125 / 1000 + player.pos:dist(minion.pos) / 840
    ...
    if (player.mana / player.maxMana) * 100 >= menu.harass.mana:get() then
        ...
        if not common.IsValidTarget(target) then return end
        if target and target.isVisible then
            ...
            if menu.harass.eharass:get() then
                if (target.pos:dist(player) < spellE.range) then
                    if menu.harass.epoison:get() then
                        if (target.buff["poisontrailtarget"] or
                            target.buff["TwitchDeadlyVenom"] or
                            target.buff["cassiopeiawpoison"] or
                            target.buff["cassiopeiaqdebuff"] or
                            target.buff["ToxicShotParticle"] or
                            target.buff["bantamtraptarget"]) then
...
local function LaneClear()
    ...
    if uhh == true then
        if (player.mana / player.maxMana) * 100 >=
            menu.laneclear.push.mana:get() then
            ...
            if menu.laneclear.push.useq:get() then
                ...
                for a = 0, minions.size[TEAM_ENEMY] - 1 do
                    ...
                    if minion1 and minion1.moveSpeed > 0 and
                        minion1.isTargetable and not minion1.isDead and
                        minion1.isVisible and
                        player.path.serverPos:distSqr(minion1.path.serverPos) <=
                        (spellQ.range * spellQ.range) then
                        local count = 0
                        for b = 0, minions.size[TEAM_ENEMY] - 1 do
                            local minion2 = minions[TEAM_ENEMY][b]
                            if minion2 and minion2.moveSpeed > 0 and
                                minion2.isTargetable and minion2 ~= minion1 and
                                not minion2.isDead and minion2.isVisible and
                                minion2.path.serverPos:distSqr(
                                    minion1.path.serverPos) <=
                                (spellQ.radius * spellQ.radius) then
                                count = count + 1
                            end
                            if count >= menu.laneclear.push.hitq:get() then
                                local seg =
                                    preds.circular.get_prediction(spellQ,
                                                                  minion1)
                                if seg and seg.startPos:dist(seg.endPos) <
                                    spellQ.range then
                                    player:castSpell("pos", 0, vec3(
                                                         seg.endPos.x,
                                                         minion1.y, seg.endPos.y))
                ...
                for i = 0, objManager.minions.size[TEAM_ENEMY] - 1 do
                    local minion = objManager.minions[TEAM_ENEMY][i]
                    if minion and minion.moveSpeed > 0 and minion.isTargetable and
                        minion.pos:dist(player.pos) <= spellQ.range and
                        minion.path.count == 0 and not minion.isDead and
                        common.IsValidTarget(minion) then
                        local minionPos = vec3(minion.x, minion.y, minion.z)
                        if minionPos then
                            if #count_minions_in_range(minionPos, 150) >=
                                menu.laneclear.push.hitq:get() then
                                local seg =
                                    preds.circular.get_prediction(spellQ, minion)
                                if seg and seg.startPos:dist(seg.endPos) <
                                    spellQ.range then
                                    player:castSpell("pos", 0, vec3(
                                                         seg.endPos.x,
            ...
            if menu.laneclear.push.farme:get() then
                ...
                for i = 0, objManager.minions.size[TEAM_ENEMY] - 1 do
                    local minion = objManager.minions[TEAM_ENEMY][i]
                    if minion and minion.moveSpeed > 0 and minion.isTargetable and
                        not minion.isDead and common.IsValidTarget(minion) then
                        local minionPos = vec3(minion.x, minion.y, minion.z)
                        if minionPos:dist(player.pos) <= spellE.range then
                            if (menu.laneclear.push.epoison:get()) then
                                if (minion.buff["poisontrailtarget"] or
                                    minion.buff["TwitchDeadlyVenom"] or
                                    minion.buff["cassiopeiawpoison"] or
                                    minion.buff["cassiopeiaqdebuff"] or
                                    minion.buff["ToxicShotParticle"] or
                                    minion.buff["bantamtraptarget"]) then
                                    player:castSpell("obj", 2, minion)
                                end
                            end
                        end
                ...
                for i = 0, objManager.minions.size[TEAM_ENEMY] - 1 do
                    local minion = objManager.minions[TEAM_ENEMY][i]
                    if minion and minion.moveSpeed > 0 and minion.isTargetable and
                        (minion.buff["poisontrailtarget"] or
                            minion.buff["TwitchDeadlyVenom"] or
                            minion.buff["cassiopeiawpoison"] or
                            minion.buff["cassiopeiaqdebuff"] or
                            minion.buff["ToxicShotParticle"] or
...
local GetPred = function()
    ...
    if res.num_hits and res.num_hits > 1 then return res.num_hits end
end
...
local function Combo()
    ...
    if target and target.isVisible then
        if mode == 1 then
            if target and (target.health / target.maxHealth) * 100 <=
                menu.combo.rset.hpr:get() and target.health >=
                menu.combo.rset.waster:get() then
                if menu.blacklist[target.charName] and
                    not menu.blacklist[target.charName]:get() then
                    local pos = preds.linear.get_prediction(spellR, target)
                    if pos and pos.startPos:dist(pos.endPos) < spellR.range and
                        #count_enemies_in_range(player.pos, 900) == 1 then
        ...
        if mode == 2 then
            if target and target.health < EDamage(target) * 3 +
                GetRDmg(target) +
                GetQDmg(target) and target.health >=
                menu.combo.rset.waster:get() then
                if menu.blacklist[target.charName] and
                    not menu.blacklist[target.charName]:get() then
                    local pos = preds.linear.get_prediction(spellR, target)
        ...
        if mode == 2 or mode == 1 then
            if GetPred() and GetPred() >= menu.combo.rset.hitr:get() then
                local pos = preds.linear.get_prediction(spellR, target)
                if pos and pos.startPos:dist(pos.endPos) < spellR.range then
                    player:castSpell("pos", 3, vec3(pos.endPos.x, mousePos.y,
                                                    pos.endPos.y))
                end
            end
        end
        if not menu.combo.wset.startw:get() and not menu.combo.rylais:get() then
                ...
                if (target.pos:dist(player) < spellQ.range) then
                    ...
                    if pos and pos.startPos:dist(pos.endPos) < spellQ.range then
                        ...
                        if menu.combo.qset.qpoison:get() then
                            if not target.buff["poisontrailtarget"] and
                                not target.buff["TwitchDeadlyVenom"] and
                                not target.buff["cassiopeiawpoison"] and
                                not target.buff["cassiopeiaqdebuff"] and
                                not target.buff["ToxicShotParticle"] and
                                not target.buff["bantamtraptarget"] then
                                player:castSpell("pos", 0, vec3(pos.endPos.x,
                                                                mousePos.y,
                                                                pos.endPos.y))
                            end
            ...
            if menu.combo.wset.wcombo:get() then
                ...
                if pos and pos.startPos:dist(pos.endPos) < spellW.range and
                    player.pos:dist(target.pos) >= 500 then
                    player:castSpell("pos", 1, vec3(pos.endPos.x, mousePos.y,
                                                    pos.endPos.y))
                end
            end
        end
        if menu.combo.wset.startw:get() then
            if menu.combo.wset.wcombo:get() then
                local pos = preds.circular.get_prediction(spellW, target)
                if pos and pos.startPos:dist(pos.endPos) < spellW.range and
                    player.pos:dist(target.pos) >= 500 then
...
function DrawDamagesE(target)
	if target.isVisible and not target.isDead then
		...
		if
			(math.floor(
				(EDamage(target) * 3 + GetQDmg(target) + GetRDmg(target)) / target.health * 100
			) < 100)
		 then
			...
			graphics.draw_text_2D(
				tostring(math.floor(EDamage(target) * 3 + GetQDmg(target) + GetRDmg(target))) ..
					" (" ..
						tostring(
							math.floor(
								(EDamage(target) * 3 + GetQDmg(target) + GetRDmg(target)) / target.health *
									100
							)
						) ..
							"%)" .. "Not Killable",
		...
		for i, enemies in ipairs(enemy) do
			if
				enemies and enemies.isVisible and common.IsValidTarget(enemies) and player.pos:dist(enemies) < 1000 and
					not common.CheckBuffType(enemies, 18)
			 then

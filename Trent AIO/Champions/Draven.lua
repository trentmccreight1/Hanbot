...
if player.charName ~= "Draven" then
    print("not using draven not loading")
    return
end
local menu = menu("Trent_Dravenxdd", "Trent Draven")
...
local function ValidTargetRange(unit, range)
    return unit and unit.isVisible and not unit.isDead and unit.isTargetable and not CheckBuffType(unit, 18) and (not range or player.pos:dist(unit.pos) <= range)
end
--
...
local function DamageR(target)
    if target ~= 0 then
        ...
        if player:spellSlot(3).state == 0 then
			Damage = (DamageAP[player:spellSlot(3).level] + 1.2 * player.baseAttackDamage + player.flatPhysicalDamageMod * 1 + player.percentPhysicalDamageMod - player.baseAttackDamage)
        end
		return Damage
	end
	return 0
...
menu.misc:boolean("GapA", "Use Stacked W for Anti-Gapclose", true)
			...
			if dasher.type == TYPE_HERO and dasher.team == TEAM_ENEMY then
				if
					dasher and common.IsValidTarget(dasher) and dasher.path.isActive and dasher.path.isDashing and
						player.pos:dist(dasher.path.point[1]) < ePred.range
				 then
...
local function IsCombo(target)
	if target.pos:dist(player.pos) <= 1200 then
		...
		if menu.combo.comboE:get() and player:spellSlot(2).state == 0 then
			...
			local seg = pred.linear.get_prediction(ePred, target)
		...
		if menu.combo.comboR:get() and player:spellSlot(3).state == 0 then
			if DamageR(target) >= target.health then
				local seg2 = pred.linear.get_prediction(rPred, target)
				if seg2 then

...
local QlvlDmg = {340, 500, 600, 820, 980}
local function qdmg(target)
    if player:spellSlot(_Q).state == 0 then
        local damage = QlvlDmg[player:spellSlot(_Q).level]
        return damage
    else
        return 0
    end
...
local function JungleFarm()
    for i = 0, objManager.minions.size[TEAM_NEUTRAL] - 1 do
        ...
        if player.pos:dist(obj) <= 1200 then
            if obj.isVisible and obj.isTargetable and obj.health >= 1 and
                common.IsValidTarget(obj) then
                if player.pos:dist(obj) <= 125 then
                    if menu.q.holdq:get() and player:spellSlot(_Q).state == 0 and minion.health <= damage then
                        player:castSpell('obj', _Q, obj)
                    else
                        if player.pos:dist(obj) <= 800 and not menu.q.holdq:get() then
                            player:castSpell('obj', _Q, obj)
                        end
                    end
                end

                if player.pos:dist(obj) <= 680 then

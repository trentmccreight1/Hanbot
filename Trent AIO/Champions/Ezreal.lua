...
if player.charName ~= "Ezreal" then
    print("not using Ezreal not loading")
    return
end
local MenuEzreal = menu("Trent_Ezrealxdd", "Trent Ezreal")
...
local TimeTurret = 0
--
local PredQ = { range = 1150; delay = 0.25; width = 60; speed = 2000;  boundingRadiusMod = 1; collision = { hero = true, minion = true, wall = true };}
local PredW = { range = 1150; delay = 0.25; width = 80; speed = 1600;  boundingRadiusMod = 1; collision = { hero = true, minion = false, wall = true };}
local PredR = { range = 3000; delay = 1.1; width = 160; speed = 2000;  boundingRadiusMod = 1; collision = { hero = false, minion = false, wall = true };}
--
...
local function ValidUlt(unit)
	if (CheckBuffType(unit, 17) or CheckBuffType(unit, 16) or CheckBuffType(unit, 18) or CheckBuff(unit, "kindredrnodeathbuff") or CheckBuffType(unit, 5)) then
		return false
	end
	return true
end
...
local function DamageQ(target)
    local Damage = 0 
    local qdmg = {20, 45, 70, 95, 120}
    if player:spellSlot(0).state == 0 then
        Damage = (qdmg[player:spellSlot(0).level] + 0.15 * (player.flatMagicDamageMod * player.percentMagicDamageMod) + 1.3 * ((player.baseAttackDamage + player.flatPhysicalDamageMod)*(1 + player.percentPhysicalDamageMod)))
    end
        ...
        for i = 0, objManager.minions.size[TEAM_ENEMY] - 1 do
            ...
            if minion and IsValidTarget(minion) then
                if ValidTargetRange(minion, q.Range) and GetDistance(vec3(minion.x, minion.y, minion.z)) > GetAARange(player) then
                    ...
                    if (lel < os.clock()) then
                        if minion.health <= DamageQ(minion) then
    ...
    if target and IsValidTarget(target) then
        if MenuEzreal.Qc.QHarass:get() and (player.mana / player.maxMana) * 100 >= MenuEzreal.Qc.ManaHarass:get() then
            if player:spellSlot(0).state == 0 and target.pos:dist(player.pos) < q.Range and #CountEnemyChampAroundObject(player.pos, 700) == 0 and target.pos:dist(player.pos) > GetAARange(player) then 
                ...
                if not PosQ3D then return end
                if not pred.collision.get_prediction(PredQ, PosQ3D, target) then

...
local function get_target(func) return ts.get_result(func).obj end
...
local function CastQt(target)
    if player:spellSlot(0).state == 0 and player:spellSlot(0).name == "GnarQ" and
        player.path.serverPos:distSqr(target.path.serverPos) < (1100 * 1100) then
        local ser = preds.linear.get_prediction(qpt, target)
        if ser and ser.startPos:dist(ser.endPos) < 1100 then
            if preds.collision.get_prediction(qpt, ser, obj) then
                return false
            end
            player:castSpell("pos", 0,
                             vec3(ser.endPos.x, game.mousePos.y, ser.endPos.y))
            --player:move(GnarQReturn.pos)
        end
    end
...
local function CastEt(target)
    if player:spellSlot(2).state == 0 and player:spellSlot(2).name == "GnarE" and
        player.path.serverPos:distSqr(target.path.serverPos) < (475 * 475) then
...
local function CastQm(target)
    if player:spellSlot(0).state == 0 and player:spellSlot(0).name == "GnarBigQ" and
        player.path.serverPos:distSqr(target.path.serverPos) < (1100 * 1100) then
        local ser = preds.linear.get_prediction(qpm, target)
        if ser and ser.startPos:dist(ser.endPos) < 1100 then
            player:castSpell("pos", 0,
...
local function CastWm(target)
    if player:spellSlot(1).state == 0 and player:spellSlot(1).name == "GnarBigW" and
        player.path.serverPos:distSqr(target.path.serverPos) < (550 * 550) then
...
local function CastEm(target)
    if player:spellSlot(2).state == 0 and player:spellSlot(2).name == "GnarBigE" and
        player.path.serverPos:distSqr(target.path.serverPos) < (600 * 600) then
...
local function CastR(target)
    if player:spellSlot(3).state == 0 and player:spellSlot(3).name == "GnarR" and
        player.path.serverPos:distSqr(target.path.serverPos) < (300 * 300) then
...
local function Combo()
    ...
    for i = 0, objManager.enemies_n - 1 do
        ...
        if enemy and common.IsValidTarget(enemy) then
            ...
            if target and common.IsValidTarget(target) then
                if menu.combo.qcombot:get() and player:spellSlot(0).state == 0 and
                    player.pos:dist(target.pos) <= qpt.range then
                    CastQt(target)
                end
                if menu.combo.ecombot:get() and player:spellSlot(2).state == 0 and
                    player.pos:dist(target.pos) <= (475 * 475) and
                    (target.health / target.maxHealth * 100) <=
                    menu.combo.ehpt:get() then
                    --[[if menu.combo.turrett:get() and
                        not common.IsUnderTurret(target) then--]]
                        CastEt(target)
                    --end
                end
                if menu.combo.qcombom:get() and player:spellSlot(0).state == 0 and
                    player.pos:dist(target.pos) <= qpm.range then
                    CastQm(target)
                end
                if menu.combo.wcombom:get() and player:spellSlot(1).state == 0 and
                    player.pos:dist(target.pos) <= wpm.range then
                    CastWm(target)
                end
                if menu.combo.ecombom:get() and player:spellSlot(2).state == 0 and
                    player.pos:dist(target.pos) <= epm.range and
                    (target.health / target.maxHealth * 100) <=
                    menu.combo.ehpm:get() then
                    --[[if menu.combo.turretm:get() and
                        not common.IsUnderTurret(object) then--]]
                        CastEm(target)
                    --end
                end
                if menu.combo.rcombo:get() and player:spellSlot(3).state == 0 and
                    menu.combo.hitr:get() <=
                    #count_enemies_in_range(target.pos, 400) and
                    player.pos:dist(target.pos) <= (300 * 300) then
                    CastR(target)
                end
            end
        end
    ...
    local function Harass()
        local target = get_target(select_target)
        if target and common.IsValidTarget(target) then
            if menu.harass.qharasst:get() and menu.keys.harasskey:get() and
                player:spellSlot(0).state == 0 and player.pos:dist(target.pos) <=
                qpt.range then CastQt(target) end
            if (target.health / target.maxHealth * 100) <= menu.combo.ehpt:get() then
                if menu.harass.eharasst:get() and menu.keys.harasskey:get() and
                    player:spellSlot(2).state == 0 and
                    player.pos:dist(target.pos) <= (475 * 475) then
                    --[[if menu.harass.turrett:get() and
                        not common.IsUnderTurret(object) then--]]
                        CastEt(target)
                    --end
                end
            end
            if menu.harass.qharassm:get() and menu.keys.harasskey:get() and
                player:spellSlot(0).state == 0 and player.pos:dist(target.pos) <=
                qpm.range then CastQm(target) end
            if menu.harass.wharassm:get() and menu.keys.harasskey:get() and
                player:spellSlot(1).state == 0 and player.pos:dist(target.pos) <=
                wpm.range then CastWm(target) end
            if (target.health / target.maxHealth * 100) <= menu.combo.ehpm:get() then
                if menu.harass.eharassm:get() and menu.keys.harasskey:get() and
                    player:spellSlot(2).state == 0 and
                    player.pos:dist(target.pos) <= epm.range then
                    --[[if menu.harass.turretm:get() and
                        not common.IsUnderTurret(object) then--]]
                        CastEm(target)
                    --end
                end
...
local function Harass()
    ...
    if target and common.IsValidTarget(target) then
        if menu.harass.qharasst:get() and menu.keys.harasskey:get() and
            player:spellSlot(0).state == 0 and player.pos:dist(target.pos) <=
            qpm.range then CastQt(target) end
        if (target.health / target.maxHealth * 100) <= menu.combo.ehpt:get() then
            if menu.harass.eharasst:get() and menu.keys.harasskey:get() and
                player:spellSlot(2).state == 0 and player.pos:dist(target.pos) <=
                (475 * 475) then
                --[[if menu.harass.turrett:get() and not common.IsUnderTurret(obj) then--]]
                    CastEt(target)
                --end
            end
        end
        ...
        if (target.health / target.maxHealth * 100) <= menu.combo.ehpm:get() then
            if menu.harass.eharassm:get() and menu.keys.harasskey:get() and
                ...
                epm.range then
                --[[if menu.harass.turretm:get() and
                    not common.is_under_tower(vec3(target.x, target.y, target.z)) then--]]
                    CastEm(target)
                --end
            end
        end
...
local function AutoQ()
    local target = get_target(select_target)
    if target and common.IsValidTarget(target) then
        if menu.harass.autoq:get() and player:spellSlot(0).state == 0 and
            player:spellSlot(0).name == "GnarQ" and player.pos:dist(target.pos) <=
            qpt.range then CastQt(target) end
    end
end
...
local function OnDraw()
    if menu.draws.drawq:get() and player:spellSlot(0).state == 0 then
        graphics.draw_circle(player.pos, qpt.range, 2, menu.draws.colorq:get(),
                             100)
    end
    if menu.draws.drawe:get() and player:spellSlot(2).state == 0 then
        graphics.draw_circle(player.pos, ept.range, 2, menu.draws.colore:get(),
                             100)
    end
    if menu.draws.drawqm:get() and player:spellSlot(0).name == "GnarBigQ" and
        player:spellSlot(0).state == 0 then
        graphics.draw_circle(player.pos, qpm.range, 2, menu.draws.colorqm:get(),
                             100)
    end
    if menu.draws.drawwm:get() and player:spellSlot(1).name == "GnarBigW" and
        player:spellSlot(1).state == 0 then
        graphics.draw_circle(player.pos, wpm.range, 2, menu.draws.colorwm:get(),
                             100)
    end
    if menu.draws.drawem:get() and player:spellSlot(2).name == "GnarBigE" and
        player:spellSlot(2).state == 0 then
        graphics.draw_circle(player.pos, epm.range, 2, menu.draws.colorem:get(),
                             100)
    end
    if menu.draws.drawr:get() and player:spellSlot(3).state == 0 then
        graphics.draw_circle(player.pos, rpm.range, 2, menu.draws.colorr:get(),
                             100)
    end
end

...
if player.charName ~= "Katarina" then
    print("not using kat not loading")
    return
end
local menu = menu("Trent_Katarinaxdd", "Trent Katarina")
...
menu.combo:boolean("eturret", " ^- Don't E under The Turret", true)
...
menu.harass:boolean("wharass", "Use W to Harass", false)
...
local QlvlDmg = {75, 105, 135, 165, 195}
local function QDamage(target)
    ...
    if player:spellSlot(0).level > 0 then
        damage =
            CalcMagicDmg(
            target,
            (QlvlDmg[player:spellSlot(0).level] + (common.GetTotalAP() * .30)) - 10
        )
    end
    return damage
end
						...
						if objs then
							...
							if #count_minions_in_range(player.pos, 450) >= menu.laneclear.hite:get() then
								...
								if menu.laneclear.turret:get() then
									if not common.IsUnderTurret(objs.pos) then
										player:castSpell("pos", 2, extendedPos)
									end
								else
									player:castSpell("pos", 2, extendedPos)
				...
				if (target.pos:dist(player.pos) <= spellE.range) then
					if not common.CheckBuffType(target, 17) then
						if (player.buff["katarinarsound"]) then
									...
									for _, objs in pairs(objHolder) do
										if objs then
											...
											if
											 ...
											 then
												if menu.combo.eturret:get() then
													if not common.IsUnderTurret(target.pos) then
														allowing = true
							...
							if QDamage(target) + EDamage(target) >= target.health and player:spellSlot(2).state == 0 then
								if (size() > 0) then
									for _, objs in pairs(objHolder) do
										if objs then
											if (target.pos:dist(objs.pos) < 450) then
...
function DrawDamagesE(target)
	if target.isVisible and not target.isDead then
		...
		if
			(math.floor(
				(RDamage(target) + EDamage(target) + PDamage(target) + QDamage(target)) / target.health * 100
			) < 100)
		 then
			...
			graphics.draw_text_2D(
				tostring(
					"R: " .. math.floor(RDamage(target) + PDamage(target) + EDamage(target) + QDamage(target))
				) ..
					" (" ..
						tostring(
							math.floor(

...
if player.charName ~= "Syndra" then
    print("not using Syndra not loading")
    return
end
local menu = menu("Trent_Syndraxdlel", "Trent Syndra")
...
local QLevelDamage = {70, 105, 140, 175, 210}
function QDamage(target)
	...
	if player:spellSlot(0).level > 0 and player:spellSlot(0).level == 5 then
		damage =
			common.CalculateMagicDamage(target, (QLevelDamage[player:spellSlot(0).level] + (common.GetTotalAP() * .65)), player) +
			common.CalculateMagicDamage(target, (QLevelDamage[player:spellSlot(0).level] + (common.GetTotalAP() * .525)), player) *
				0.1625
	end
	return damage
end
...
function WDamage(target)
    ...
    if player:spellSlot(1).level > 0 then
        damage =
            common.CalculateMagicDamage(target, (WLevelDamage[player:spellSlot(1).level] + (common.GetTotalAP() * .70)), player)
    end
    return damage
end

local ELevelDamage = {85, 130, 175, 220, 265}
function EDamage(target)
    local damage = 0
    if player:spellSlot(2).level > 0 then
        damage =
            common.CalculateMagicDamage(target, (ELevelDamage[player:spellSlot(2).level] + (common.GetTotalAP() * .60)), player)
    end
    return damage
end
...
local function Killsteal()
	...
	for i, enemies in ipairs(enemy) do
		if enemies and enemies.isVisible and common.IsValidTarget(enemies) and not common.CheckBuffType(enemies, 18) then
	...
	if os.clock() - LastWCast > 0.1 + network.latency then
		if menu.combo.ecombo:get() then
			...
			for i, target in ipairs(enemy) do
				if target and target.isVisible and common.IsValidTarget(target) and not common.CheckBuffType(target, 18) then
					if common.IsValidTarget(target) then
						...
						if menu.blacklist[target.charName] and not menu.blacklist[target.charName]:get() then
							if player:spellSlot(3).stacks >= menu.combo.rset.orb:get() then
								...
								if menu.combo.rset.engagemode:get() then
									local damages =
										RDamage(target) + QDamage(enemies) + WDamage(target) + EDamage(target)
				...
				if target and target.isVisible then
					if common.IsValidTarget(target) and player.mana > player.manaCost0 + player.manaCost2 then
						if (target.pos:dist(player.pos) <= spellQE.range) then
							...
							if
								(target.path.count > 0) or
									(target.buff[6] or target.buff[9] or target.buff[25] or target.buff[12] or target.buff[23] or target.buff[9] or
										target.buff[22])
							 then
								local pos = preds.linear.get_prediction(spellQE2, target)
								if pos and pos.startPos:dist(pos.endPos) <= spellQE.range then

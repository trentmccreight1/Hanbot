...
local trace_filter = function(input, segment, target)
    if segment.startPos:dist(segment.endPos) > qPred.range then
        return false
    end -- Checking before spellcast to see if our projected segment end is out of range if not return
    if preds.trace.linear.hardlock(input, segment, target) then
        return true
    end -- checking cc status if so send it no real pred needed
    if preds.trace.linear.hardlockmove(input, segment, target) then -- checking cc status if so send it no real pred needed
        return true
    end
    if preds.trace.newpath(target, 0.05445, 1.0) then
        return true
    end -- Kiri path strictness :omegalul:
end
...
local function GetQDmg(unit, pos)
    if player:spellSlot(0).level < 1 or not unit or unit.isDead or not unit.isVisible or not unit.isTargetable then
        return 0
    end
    local d = pos and player.path.serverPos:dist(pos) or player.path.serverPos:dist(unit.path.serverPos)
    local pctIncrease = 1
    if d >= 622 then
        if d >= 1300 then
            pctIncrease = 3
...
local function FleeKiteLogic()
    ...
    if not isHuman() then
        if player:spellSlot(0).name ~= "JavelinToss" and player:spellSlot(3).state == 0 then
            player:castSpell("self", 3)
            print("Used R to Human")
        else
            if player:spellSlot(1).name == "Pounce" and player:spellSlot(1).state == 0 then
...
local function GetWHuman(target)
    if target ~= 0 then
        ...
        if player:spellSlot(1).state == 0 then
            Damage =
                (DamageAP[player:spellSlot(1).level] + 0.2 * player.flatMagicDamageMod * player.percentMagicDamageMod)
        end
        return Damage
    end
    return 0
...
local function GetWCour(target)
    if target ~= 0 then
        ...
        if player:spellSlot(1).state == 0 then
            Damage =
                (DamageAP[player:spellSlot(3).level] + 0.3 * player.flatMagicDamageMod * player.percentMagicDamageMod)
        end
        return Damage
    end
    return 0
...
local function Farm()
    ...
    for i = 0, minions.size[TEAM_ENEMY] - 1 do
        ...
        if minion and common.IsValidTarget(minion) then
            ...
            if not isHuman() then
                if
                    menu.farm.QC:get() and player:spellSlot(0).state == 0 and player:spellSlot(0).name ~= "JavelinToss" and
                        dist <= 400
                 then
                    player:castSpell("self", 0)
                end
                if menu.farm.WC:get() and player:spellSlot(1).state == 0 and player:spellSlot(1).name == "Pounce" then
                    if dist <= 750 then
                        player:castSpell("pos", 1, minion.pos)
                    end
                end
                if menu.farm.EC:get() and player:spellSlot(2).state == 0 and player:spellSlot(2).name == "Swipe" then
                ...
                if menu.farm.autoR:get() and player:spellSlot(3).state == 0 then
                    if
                        player:spellSlot(0).state ~= 0 and player:spellSlot(0).name ~= "JavelinToss" and
                            ...
                            player:spellSlot(2).name == "Swipe" and
                            manaCheck
                     then
                        player:castSpell("self", 3)
                    end
            ...
            if isHuman() then
                if
                    menu.farm.Q:get() and player:spellSlot(0).state == 0 and player:spellSlot(0).name == "JavelinToss" and
                        manaCheck and
                        dist <= 1500
                 then
                ...
                if
                    menu.farm.W:get() and player:spellSlot(1).state == 0 and player:spellSlot(1).name == "BushWhack" and
                        dist < 900 and
                        manaCheck
                 then
                    if GetWHuman(minion) > minion.health and not minion.pathssActive then
                        player:castSpell("pos", 1, minion.pos)
                    end
                end
                if
                    menu.farm.autoR:get() and player:spellSlot(3).state == 0 and
                        (player:spellSlot(0).state ~= 0 and player:spellSlot(0).name == "JavelinToss" or not manaCheck or
                            not menu.farm.Q:get())
                 then
                    player:castSpell("self", 3)
                end
    ...
    for i = 0, minions.size[TEAM_NEUTRAL] - 1 do
        ...
        if minion and common.IsValidTarget(minion) then
            ...
            if not isHuman() then
                ...
                if menu.farm.autoR:get() and player:spellSlot(3).state == 0 then
                    if player:spellSlot(0).state ~= 0 and player:spellSlot(0).name ~= "JavelinToss" and manaCheck then
                        player:castSpell("self", 3)
                    end
                end
            end
...
local function Healing()
    ...
    if player:spellSlot(2).state == 0 and player:spellSlot(2).name ~= "Swipe" then
        if
            menu.heal.useSelf:get() and common.GetPercentHealth(obj) < menu.heal.healthSelf:get() and
                common.GetPercentPar(obj) > menu.heal.Mana1:get()
         then
            if isHuman() and player:spellSlot(2).state == 0 then
                player:castSpell("self", 2)
            end
            if
                not isHuman() and menu.heal.autoR:get() and player:spellSlot(3).state == 0 and not orb.combo.is_active() and
                    not orb.menu.lane_clear:get()
             then
                player:castSpell("self", 3)
        ...
        if menu.heal.useAlly:get() and common.GetPercentPar(obj) > menu.heal.Mana2:get() then
            for i = 0, objManager.allies_n - 1 do
                ...
                if ally and not ally.isDead and ally.isVisible then
                    ...
                    if dist <= 600 and common.GetPercentHealth(ally) < menu.heal.healthAlly:get() then
                        if isHuman() and player:spellSlot(2).state == 0 then
                            player:castSpell("obj", 2, ally)
                        end
                        if
                            not isHuman() and menu.heal.autoR:get() and player:spellSlot(3).state == 0 and
...
local function KS()
    if not menu.ks.Q:get() or player:spellSlot(0).state ~= 0 or player:spellSlot(0).name ~= "JavelinToss" then
        return
    end
    for i = 0, objManager.enemies_n - 1 do
        local enemy = objManager.enemies[i]
        if enemy and common.IsValidTarget(enemy) and enemy.pos:dist(player.pos) <= 1500 then
            if isHuman() and player:spellSlot(0).state == 0 and player:spellSlot(0).name == "JavelinToss" then
...
local function Combo()
    ...
    if target and common.IsValidTarget(target) then
        ...
        if not isHuman() then
            ...
            if menu.combo.QC:get() and player:spellSlot(0).state == 0 and player:spellSlot(0).name == "Takedown" and dist < 400 then
                player:castSpell("self", 0)
                orb.core.reset()
            end
            if menu.combo.RC:get() and player:spellSlot(3).state == 0 then
                ...
                if isHunted(target) and dist > 750 and player.par >= manaSpear and player:spellSlot(0).state == 0 then
                    player:castSpell("self", 3)
                elseif dist >= 375 then
                    player:castSpell("self", 3)
                elseif
                    player:spellSlot(0).state ~= 0 and player:spellSlot(1).state ~= 0 and player:spellSlot(2).state ~= 0
                 then
...
local function OnDraw()
    if isHuman() then
        if menu.draws.Q:get() and player:spellSlot(0).state == 0 and player.isOnScreen then
            graphics.draw_circle(player.pos, 1500, 2, graphics.argb(255, 7, 141, 237), 50)
        end
        if menu.draws.W:get() and player:spellSlot(1).state == 0 and player.isOnScreen then
            graphics.draw_circle(player.pos, 900, 2, graphics.argb(255, 7, 141, 237), 50)
        end
    else
        if menu.draws.QC:get() and player:spellSlot(0).state == 0 and player.isOnScreen then
            graphics.draw_circle(player.pos, 325, 2, graphics.argb(255, 7, 141, 237), 50)
        end
        if menu.draws.WC:get() and player:spellSlot(1).state == 0 and player.isOnScreen then
            graphics.draw_circle(player.pos, 375, 2, graphics.argb(255, 7, 141, 237), 50)
        end
        if menu.draws.EC:get() and player:spellSlot(2).state == 0 and player.isOnScreen then
            graphics.draw_circle(player.pos, 300, 2, graphics.argb(255, 7, 141, 237), 50)
        end
    end
end
